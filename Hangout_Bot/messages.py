...
def error_handler(event, phase_num):
    ...
    if phase_num == NAME:
        if (len(message) > 0 and message != error_dictionary[0]):
            ...
            if (get_campaign_data(user_key) == None):
                ...
                return 'The campaign name, \"{}\", already exists!'
                    .format(message)
        else:
    ...
    # phase 1: start date, phase 2: end date
        ...
        if len(date_parameters) == 3:
            if (int(date_parameters[0]) >= JANUARY) and
                (int(date_parameters[0]) <= DECEMBER):
                if ((int(date_parameters[1]) > 0) and
                    (int(date_parameters[1]) <= LAST_DAY_MONTH - 1)) or
                    ((int(date_parameters[0]) % 2 == 0) and
                    (int(date_parameters[1]) == LAST_DAY_MONTH)):
                    if ((int(date_parameters[2]) >= 2000) and
                        (int(date_parameters[2]) <= 2099)):
                        return success
                    else:
                        return 'Year can only be between 2000 and 2099!'
                else:
                    return 'Day must be between 1 and 31! If you entered 31' +
                           ' as a day, make sure the month has 31 days!'
            else:
                return 'Month must be between 1 and 12!'
        return 'Date is not in the right format! Must be in the form ' +
               'mm-dd-yyyy'

    # phase 3: daily budget, phase 8: manual CPC
    elif phase_num == DAILY_BUDGET or phase_num == MANUAL_CPC:
        attribute_name = 'Daily budget' if phase_num == 3
                          else 'Manual CPC (Cost Per Click)'
        begin_index = 0
        if '$' in message:
        ...
        try:
          ...
          if (Decimal(message) >= error_dictionary[3]): 
           ...
           return '{} must be a valid decimal value! Ex: $34.00'
    ...
    # phase 4: locations, phase 5: negative locations
        ...
        for idx in range(len(locations)):
            ...
            if phase_num == NEG_LOCATIONS:
                ...
                if locations[idx] in location_data:
                    return '{} is already a targetted location, it cannot ' +
                           'be excluded.'.format(locations[idx])
        ...
        if (len(locations) == 0):
            return 'Locations must be a comma separated list of state codes.' +
                   ' Ex: CA, TX, MN, PA'
        return success
